<template>
  <v-toolbar dark color="#151621" elevation="0" class="pt-3 mb-8">
    ...
    <v-toolbar-title>
      <v-breadcrumbs class="pl-0" :items="items.filter(item => item.to != '/admin').map(item => ({
        text: item.text, to: item.to, exact: true
      }))"
      />
    </v-toolbar-title>
  </v-toolbar>
...
export default {
  ...
  computed: {
    active () {
      return this.items.find(item => item.active).text
    }
  },
  created() {
